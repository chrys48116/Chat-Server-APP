defmodule ChatServer do
  ...
  def create_room(room_name), do: GenServer.call(__MODULE__, {:create_room, room_name})

  def add_user(user_id, username), do: GenServer.call(__MODULE__, {:add_user, user_id, username})

  def remove_user(user_id), do: GenServer.call(__MODULE__, {:remove_user, user_id})

  def send_group_message(%{room_name: room_name, user_id_from: user_id_from, content: content}) do
    GenServer.call(__MODULE__, {:send_group_message, room_name, user_id_from, content})
  end
  ...
  def receive_messages(room_name), do: GenServer.call(__MODULE__, {:receive_messages, room_name})

  def get_pending_messages(user_id), do: GenServer.call(__MODULE__, {:get_pending_messages, user_id})

  def join_room(user_id, room_name), do: GenServer.call(__MODULE__, {:join_room, user_id, room_name})
  ...
  def handle_call({:create_room, room_name}, _from, state) do
    ...
    {:reply, "Group #{room_name} created", %{state | rooms: room}}
  end

  def handle_call({:join_room, user_id, room_name}, _from, state), do:
    then(Logic.join_room(user_id, room_name, state), fn {_status, response, state} ->
      {:reply, response, state}
    end)

  def handle_call({:send_group_message, room_name, user_id_from, content}, _from, state) do
    then(Logic.send_group_message(room_name, user_id_from, content, state), fn {_status, response, state} ->
      {:reply, response, state}
    end)
  end

  def handle_call({:send_message, user_id_from, user_id_to, content}, _from, state) do
    then(Logic.send_message(user_id_from, user_id_to, content, state), fn {_status, response, state} ->
      {:reply, response, state}
    end)
  end

  def handle_call({:receive_messages, room_name}, _from, state), do:
    then(Logic.receive_messages(room_name, state), fn {_status, response, state} ->
      {:reply, response, state}
    end)

  def handle_call({:get_pending_messages, user_id}, _from, state) do
    then(Logic.get_pending_messages(user_id, state), fn {_status, response, state} ->
      {:reply, response, state}
    end)
  end

  def handle_call({:add_user, user_id, username}, _from, state), do:
    then(Logic.add_user(user_id, username, state), fn {_status, response, state} ->
      {:reply, response, state}
    end)

  def handle_call({:remove_user, user_id}, _from, state), do:
    then(Logic.remove_user(user_id, state), fn {_status, response, state} ->
      {:reply, response, state}
    end)
end
